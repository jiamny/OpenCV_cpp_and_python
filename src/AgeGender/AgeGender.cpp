...
int main(int argc, const char* argv[]) {
	...
	const char* n_argv[] = {"AgeGender", "./src/AgeGender/sample1.jpg", "cpu"};
	argc = 3;
	argv = n_argv;

	std::cout << "Current path is " << get_current_dir_name() << '\n';

    string faceProto = "./models/AgeGender_model/opencv_face_detector.pbtxt";
    string faceModel = "./models/AgeGender_model/opencv_face_detector_uint8.pb";

    string ageProto = "./models/AgeGender_model/age_deploy.prototxt";
    string ageModel = "./models/AgeGender_model/age_net.caffemodel";

    string genderProto = "./models/AgeGender_model/gender_deploy.prototxt";
    string genderModel = "./models/AgeGender_model/gender_net.caffemodel";
...
//    while(waitKey(1) < 0) {
      // read frame
      Mat frame = imread(videoFile, cv::IMREAD_COLOR);
//      cap.read(frame);
      if (frame.empty()) {
          waitKey();
//          break;
          std::cout << "Can't find image file\n";
          return 1;
      }
...
//        continue;
      ...
      for (auto it = begin(bboxes); it != end(bboxes); ++it) {
        ...
        */
        ...
        waitKey(-1);
        imwrite("./src/AgeGender/out.jpg",frameFace);
      }
//    }
      return 0;
}
