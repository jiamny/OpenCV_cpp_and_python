...
//"{image i        |./src/AugmentedRealityWithArucoMarkers/test.jpg| input image   }"
"{image i        |<none>| input image   }"
"{video v       |./src/AugmentedRealityWithArucoMarkers/video/test.mp4| input video   }"
;
using namespace cv;
...
int main(int argc, char** argv) {
    ...
    std::cout << "has: " << parser.has("image") << std::endl;
    ...
    Mat im_src = imread("./src/AugmentedRealityWithArucoMarkers/image/new_scenery.jpg");

    try {
        
        outputFile = "./src/AugmentedRealityWithArucoMarkers/ar_out_cpp.avi";
        ...
        if(parser.has("image")){
            ...
            std::cout << "output video file: " << str << std::endl;
            ifstream ifile(str);
            if (!ifile) throw("error");
            cap.open(str);
